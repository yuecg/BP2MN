...
from .bp2mn_modules import CNN, LGTMBlock, deform_inputs

_logger = logging.getLogger(__name__)


@BACKBONES.register_module()
class BP2MN(TIMMVisionTransformer):
    def __init__(self, pretrain_size=224, num_heads=12, conv_inplane=64, n_points=4, deform_num_heads=6,
                 ...
                 deform_ratio=1.0, add_vit_feature=True, use_extra_LGTM=True, pretrained=None,with_cp=False,
                 use_LGTM_toV=True,
                 use_LGTM_toC=True,
        ...
        self.interactions = nn.Sequential(*[
            LGTMBlock(dim=embed_dim, num_heads=deform_num_heads, n_points=n_points,
                            init_values=init_values, drop_path=self.drop_path_rate,
                            norm_layer=self.norm_layer, with_cffn=with_cffn,
                            cffn_ratio=cffn_ratio, deform_ratio=deform_ratio,
                            use_LGTM_toV=use_LGTM_toV if isinstance(use_LGTM_toV, bool) else use_LGTM_toV[i],
                            use_LGTM_toC=use_LGTM_toC if isinstance(use_LGTM_toC, bool) else use_LGTM_toC[i],
                            dim_ratio=dim_ratio,
                            ...
                            extra_LGTM=((True if i == len(interaction_indexes) - 1 else False) and use_extra_LGTM))
